import datetime, bisect, time, json, os
...
def get_active_macs():
        myfile = open(r'/var/lib/dhcpd/dhcpd.leases', 'r')
        leases = parse_leases_file(myfile)
        myfile.close()

        now = timestamp_now()
        report_dataset = select_active_leases(leases, now)
        active_macs = [lease['hardware'] for lease in report_dataset]
        return active_macs
...
while True: 
        try:
                ...
                active_macs = list(set([x for x in active_macs if x!='']))
                print(active_macs)
                #print(data.keys())
                ...
                for active_mac in active_macs:
                        data[active_mac]["Status"] = "ACTIVE"
                
                with open("devices.json",'w') as file:
                        json.dump(data,file,indent=4)

        except Exception:
                os.system("touch devices.json")
